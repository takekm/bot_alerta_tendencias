...
def get_company_name(ticker: str) -> str:
    """Devuelve nombre amigable de la compañía/activo para un ticker."""
    if ticker in NOMBRE_CACHE:
        return NOMBRE_CACHE[ticker]

    nombre = ticker  # fallback

    try:
        t = yf.Ticker(ticker)
        info = getattr(t, "fast_info", None)  # a veces existe, pero no siempre trae name
        # Mejor: info clásico
        inf = t.info or {}
...
def agregar_registro(a_que_lista, ticker, señal, cierre, Var_nominal, Var_pct, cruce ,racha, RSI14,dolar_ccl,dif_ccl_vs_mercado):
    a_que_lista.append({
        "Compañía": get_company_name(ticker),
        "Ticker": ticker,
        "Señal": señal,
        "Cierre": cierre,
        "vs LD $":Var_nominal,
...
# Cache para no llamar mil veces a Yahoo
NOMBRE_CACHE = {}
...
def construir_tabla_html(registros, titulo):
    ...
    <thead>
      <tr>
        <th style="text-align:left; padding:8px; border-bottom:1px solid #e5e7eb;">Compañía</th>
    ...
    for r in registros:
        ...
        <tr style="background:{bg};">
            <td style="padding:8px; border-bottom:1px solid #f3f4f6; max-width:260px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
            {escape(str(r.get('Compañía','—')))}
            </td>
